...
- Expand HK share-authorization to include `HKWorkoutType` (currently only `workoutEffortScore`).
- New `HealthKitManager.writeWorkout(...)` that builds and saves an `HKWorkout` from a `StravaActivity` plus the data we can pull from Strava: start, end, activity type, distance, total energy burned (calories), elevation gain. HR-summary metadata (`avg`/`max` HR) attached as workout metadata; per-sample HR written as `HKQuantitySample` of type `.heartRate` covering the workout's time range (see HR streams below).
- New `StravaClient.fetchActivityDetail(id:)` hitting `/activities/{id}` — needed for `calories`, which is detail-only.
...
4. `HealthKitManager.writeWorkout(...)` that builds an `HKWorkout` from a `StravaActivity` + detail fields + HR-stream samples. Round-trip test: save, query back, assert fields + HR sample count.
